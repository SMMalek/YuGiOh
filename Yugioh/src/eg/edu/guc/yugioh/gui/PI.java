...
public class PI extends JFrame {
	...
	public PI(Player Cont) {
		...
		ActionListener Summm = new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				for (int i = play.getField().getHand().size() - 1; i >= 0; i--) {
					...
					if (play.getField().getHand().get(i).getName().equals((O))) {
						// play.getField().getPhase();
						System.out.println("IN");
						play.summonMonster((MonsterCard) play.getField()
								.getHand().get(i));
					}
				}
				System.out.println(play.getField().getMonstersArea().size());
				// System.out.println(play.getField().getMonstersArea().get(0).getName());
			}
		};
		...
		Sum.addActionListener(Summm);  //LINE 
	...
	private void refM() {
		...
		for (int i = 0; i != x.size(); i++) {
			String l = x.get(i).getName();
			JButton t = new JButton(l);
			ImageIcon im = new ImageIcon(l + ".png");
			t.setIcon(im);
	...
	private void refH() {
		...
		for (int i = 0; i != addthismons.size(); i++) {
			ButtonName = addthismons.get(i).getName();
			JButton addedmonsButton = new JButton(ButtonName);
			ImageIcon addedmonsimage = new ImageIcon(ButtonName + ".png");
			addedmonsimage.setImage(addedmonsimage.getImage()
					.getScaledInstance(170, 130, 20));
			addedmonsButton.setIcon(addedmonsimage);
			addedmonsimage.setDescription(addthismons.get(i).getDescription());
